...
def setProxy():
    http.client.HTTPSConnection("127.0.0.1", 10809)
    socks.set_default_proxy(socks.SOCKS5, '127.0.0.1', 10808)
    socket.socket = socks.socksocket

    proxy_iplist = ['127.0.0.1:10809']
...
# 总是重试时，apiclient.错误。带有这些状态码之一的HttpError被提升。
...
# 这个变量定义了一条消息，如果CLIENT_SECRETS_FILE丢失，它将显示一条消息
...
def get_authenticated_service(args):
    ...
    return build(YOUTUBE_API_SERVICE_NAME, YOUTUBE_API_VERSION,
                 ...
                 #     proxy_info=httplib2.ProxyInfo(socks.SOCKS5, '127.0.0.1', 10808))))
                 http=credentials.authorize(httplib2.Http()))
...
def main(_file, _title, _description, _category, _keywords):
...
#     args = parser.parse_args(["--file", _file],
#                              ["--title",_title],
